...
// src/repository/RedisRepository.ts
...
var CardSchema = z.object({
  card_number: z.string().min(13, { message: "Card number must be at least 13 digits" }).max(16, { message: "Card number must be no longer than 16 digits" }).refine(luhnAlgorithm, { message: "Invalid card number" }),
  cvv: z.string().min(3, { message: "CVV must be at least 3 digits" }).max(4, { message: "CVV must be no longer than 4 digits" }),
  expiration_month: z.string().min(1, { message: "Month must be at least 1 digit" }).max(2, { message: "Month must be no longer than 2 digits" }).refine((val) => Number(val) >= 1 && Number(val) <= 12, { message: "Invalid month" }),
  expiration_year: z.string().length(4, { message: "Expiration year must be 4 digits" }).refine(
    (val) => {
  ...
  email: z.string().min(5, { message: "Email must be at least 5 characters" }).max(100, { message: "Email must be no longer than 100 characters" }).email({ message: "Invalid email format" }).refine((val) => /^(gmail\.com|hotmail\.com|yahoo\.es)$/.test(val.split("@")[1]), {
...
function responseFactory(type, body) {
  ...
  if (type === 404 /* NotFound */)
    return { statusCode: 404 /* NotFound */, body: parsedBody };
...
var TokenService = class {
  ...
  async tokenize(event) {
    ...
    try {
      ...
      const token = import_crypto.default.randomBytes(8).toString("hex");
      try {
  ...
  async getCardDetailsByToken(event) {
    try {
      this.validateAuthorization(event.headers);
    } catch (error) {
      return responseFactory(401 /* Unauthorized */, { error: error.message });
    ...
    if (!token)
      return responseFactory(400 /* BadRequest */, { message: "No token provided" });
    try {
      const cardDetails = await this.redisRepository.get(token);
      if (!cardDetails)
        return responseFactory(404 /* NotFound */, { message: "Card not found" });
      return responseFactory(200 /* Ok */, { card: JSON.parse(cardDetails) });
    } catch (error) {
      return responseFactory(500 /* InternalServerError */, { error });
    }
  }
};
